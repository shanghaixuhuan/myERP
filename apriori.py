...
class Apriori():
    ...
    def apriori(dataSet, minSupport=0.3):
        ...
        L1, supportData = Apriori.scanD(D, C1, minSupport)  # 过滤数据
        L = [L1]
        k = 2
        while (len(L[k - 2]) > 0):  # 若仍有满足支持度的集合则继续做关联分析
            Ck = Apriori.aprioriGen(L[k - 2], k)  # Ck候选频繁项集
            Lk, supK = Apriori.scanD(D, Ck, minSupport)  # Lk频繁项集
    ...
    def printData(L,suppData):
        f = open('result\\result.DAT','w')
        for j in range(len(L[1])):
            f.write(str(L[1][j])[11:][:-2]+'\n')
        f.close()
